...
library(pacman)
p_load(dplyr, stringr, readxl, janitor, tidyverse, readxl)
rm()
```

# NOTE: 
Baruch: Drop 1 of pavement ranking (urban )? Currently 4
Fatality: Need to include more roads. 
...
# Combining data & Calculating key metrics
```{r}
...
#Calculate key metrics
  mutate(
    ...
    # previous approach: using per lane miles
    ...
    rural_fatalities_per_100m_VMT = rural_I_OFE_OPA_fatalities / rural_VMT * 100, # numerator: should use total
...
#Calculate overall scores using 3 per_lm + NOT including cost of living
AHR_data_composite_perlm_NOcoli <- AHR_data %>% 
  pivot_longer(cols = state_avg_delay_hours : urban_fatalities_per_100m_VMT, #pivot col 27: state_avg_delay_hours
               names_to = "key_metrics", 
               values_to = "value") %>%  
  arrange(key_metrics) %>% 
  filter(!key_metrics %in% c("coli_index", "capital_disbursement_per_vmt", "maintenance_disbursement_per_vmt", "admin_disbursement_per_vmt")) %>% 
  ...
  ungroup() %>% 
  group_by(state) %>% 
  summarise(overall_score_perlm_NOcoli = mean(relative_score, na.rm = T)) %>% 
  ungroup()

```

```{r}
#Calculate overall scores using VMT +  NOT including cost of living
AHR_data_composite_NOcoli <- AHR_data %>% 
  ...
  # overall_score NOT adjusted for COLI ; removing per-lm
  filter(!key_metrics %in% c("coli_index", "capital_disbursement_per_lm", "maintenance_disbursement_per_lm", "admin_disbursement_per_lm")) %>%  
  select(state, key_metrics, value) %>% 
  group_by(key_metrics) %>% 
...
```{r}
  ...
  # overall_score using VMT + adjusted for COLI  
AHR_data_composite_WITHcoli <- AHR_data %>%  
  ...
  mutate(admin_disbursement_per_vmt = admin_disbursement_per_vmt/coli_index, 
  ...
  #removing per-lm
  filter(!key_metrics %in% c("capital_disbursement_per_lm", "maintenance_disbursement_per_lm", "admin_disbursement_per_lm")) %>%  
...
AHR_data_composite <- AHR_data_composite_NOcoli %>% left_join(AHR_data_composite_WITHcoli) %>% left_join(AHR_data_composite_perlm_NOcoli)
...
#Rank states according to the calculated metrics
  ...
  mutate(
    across(c(state_avg_delay_hours:overall_score_perlm_NOcoli), min_rank, .names = "{.col}_rank")
  ) 

#final

AHR_data_umr_coli <- AHR_data %>% 
  left_join(AHR_data_rank)

write.csv(AHR_data_umr_coli, "AHR_data_umr_coli.csv")
```
