...
HM_10 <- import("data/2022/hm10.xls", sheet = "A") %>% 
  remove_empty() %>% 
  select(1, 2, 5, 8, 11) %>% 
  slice(-(1:6)) %>% 
  rename(state = 1,
...
HM_81 <- import("data/2022/hm81.xls", sheet  = "A") %>% 
  remove_empty() %>% 
  select(1, 17) %>% 
  rename(state = 1,
...
SF_4 <- import("data/2022/sf4.xlsx", sheet = "A") %>% 
  remove_empty() %>% 
  select(1:4, 8) %>% 
  rename(state = 1,
...
HM64_rural_interstate <- import("data/2022/hm64.xls", sheet = "A") %>% 
  remove_empty() %>% 
  select(1, 8:11) %>% 
  rename(state = 1,
...
HM64_rural_OPA <- import("data/2022/hm64.xls", sheet = "B") %>% 
  remove_empty() %>% 
  select(1, 20:21) %>% 
  rename(state = 1,
...
HM64_urban_interstate <- import("data/2022/hm64.xls", sheet = "C") %>% 
  remove_empty() %>% 
  select(1, 8:11) %>% 
  rename(state = 1,
...
HM64_urban_OPA <- import("data/2022/hm64.xls", sheet = "D") %>% 
  remove_empty() %>% 
  select(1, 20:21) %>% 
  rename(state = 1,
...
VM_2 <- read_excel("data/2022/vm2.xls", sheet = "A") %>%
  select(1:4, 10:12, 18) %>% 
  rename(state = 1,
