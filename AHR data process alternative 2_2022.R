...
HM_10 <- read_excel("data/2022/hm10.xls", sheet = "A") %>% 
  remove_empty() %>% 
  select(1, 2, 5, 8, 11) %>% 
  slice(-(1:6)) %>% 
  rename(state = 1,
...
HM_81 <- read_excel("data/2022/hm81.xls", sheet  = "A") %>% 
  remove_empty() %>% 
  rename(state = 1,
...
SF_4 <- read_excel("data/2022/sf4.xlsx", sheet = "A") %>% 
  remove_empty() %>% 
  select(1:7) %>% 
  rename(state = 1,
...
HM64_rural_interstate <- read_excel("data/2022/hm64.xls", sheet = "A") %>% 
  remove_empty() %>% 
  select(1, 8:11) %>% 
  rename(state = 1,
...
HM64_rural_OPA <- read_excel("data/2022/hm64.xls", sheet = "B") %>% 
  remove_empty() %>% 
  select(1, 20:21) %>% 
  rename(state = 1,
...
HM64_urban_interstate <- read_excel("data/2022/hm64.xls", sheet = "C") %>% 
  remove_empty() %>% 
  select(1, 8:11) %>% 
  rename(state = 1,
...
HM64_urban_OPA <- read_excel("data/2022/hm64.xls", sheet = "D") %>% 
  remove_empty() %>% 
  select(1, 20:21) %>% 
  rename(state = 1,
...
#TODO: Checked March 13, not yet available: https://www.fhwa.dot.gov/policyinformation/statistics/2022/
...
VM_2 <- read_excel("data/2022/vm2.xls", sheet = "A", skip = 13) %>% 
  rename(state = 1) %>% 
...
rio::export(data_list, "output/AHR_2022_final.xlsx")
