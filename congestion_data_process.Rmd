...
#-------------------------
# the plot shows that the relationship between auto_commuters_combined and congestion_hours is not linear. 
#Most points are in the lower left corner; only few points in the upper right. 
# --> the result might cause inflated (biased )  values. 
# Possible solution: impute Na values of congestion hours by the mean of its predictor in each tile 
# ??? Question: Is there any other predictor of congestion hours?

# the missing values: 195/390 = 50% --> This is a lot of missing values. Imputing values using only 1 predictor is a bold move. The most conservative/secured way is remove these NAs value.  Other than that, here is one approach:
#1. partition auto_commuters_combined into 10 tiles
#2. calculate mean of congestion_hours in each tile
#3. impute NAs values in congestion_hours by the mean of its tile

# Demonstrated below for easy view:

congestion_data %>% select(city, auto_commuters_combined, congestion_hours) %>% arrange(desc(auto_commuters_combined)) %>% 
  mutate(ntile_auto_commuters_combined = ntile(auto_commuters_combined, 10)) %>%   # partition auto_commuters_combined data in to 10 tiles 
  group_by(ntile_auto_commuters_combined) %>% 
  mutate(avg_ntile = mean(congestion_hours, na.rm = TRUE)) %>% 
  mutate(congestion_hours_imputed = ifelse(is.na(congestion_hours), avg_ntile, congestion_hours))

```

```{r}
#-------------------THIS CHUNK IS ONLY FOR DEMONSTRATION
...
#method 2: using mean of each tile
  
congestion_data_non_inrix_tile_imputed <- congestion_data %>%
mutate(ntile_auto_commuters_combined = ntile(auto_commuters_combined, 10)) %>%   # partition auto_commuters_combined data in to 10 tiles 
  group_by(ntile_auto_commuters_combined) %>% 
  mutate(avg_ntile = mean(congestion_hours, na.rm = TRUE)) %>% 
  mutate(congestion_hours_tile_imputed = ifelse(is.na(congestion_hours), avg_ntile, congestion_hours)) %>% 
  filter(is.na(congestion_hours))
  
# compare results of 2 methods
congestion_data_non_inrix$congestion_hours_tile_impute <- congestion_data_non_inrix_tile_imputed$congestion_hours_tile_imputed
congestion_data_non_inrix %>% arrange(desc(auto_commuters_combined))
````


```{r}
##### The following parts keep using method 2.

# add the daily vehicle miles traveled data to allocate the commuter number for multi-state areas
congestion_data_final <- congestion_data %>%
mutate(ntile_auto_commuters_combined = ntile(auto_commuters_combined, 10)) %>%   # partition auto_commuters_combined data in to 10 tiles 
  group_by(ntile_auto_commuters_combined) %>% 
  mutate(avg_ntile = mean(congestion_hours, na.rm = TRUE)) %>% 
  mutate(congestion_hours_tile_imputed = ifelse(is.na(congestion_hours), avg_ntile, congestion_hours)) %>% 
  ...
  mutate(dmvt_pct = ifelse(is.na(dmvt_pct), 1, dmvt_pct),
         ...
         total_congestion_hours = congestion_hours_tile_imputed * auto_commuters_combined * dmvt_pct,
         adjusted_auto_commuters_combined = auto_commuters_combined * dmvt_pct)

#Calculate congestion hours per commuter by state
congestion_data_summary_tile <- congestion_data_final %>% 
  group_by(state) %>% 
...
saveRDS(congestion_data_summary_tile, "congestion_data_summary_tile.RDS")
